"""
train_dr.py — SAC Training Script for Domain Randomized Goal-Conditioned Hover
================================================================================
Trains a Domain Randomized SAC agent to hover a Crazyflie v2 at any
target height z ∈ [0.2, 2.5] m, while subject to random motor efficiency
degradation (60% to 100%) during training.

Usage:
    python3 train_dr.py                     # default 2M steps
    python3 train_dr.py --steps 1000000     # custom step count
    python3 train_dr.py --gui               # visual debug

Features:
  • Domain Randomization on motor thrust efficiency
  • TensorBoard logging includes custom/mean_thrust_factor
...
# Hyper-parameters
# ──────────────────────────────────────────────────────────────────────────────
TOTAL_TIMESTEPS = 2_000_000       # Total training timesteps for DR agent
...
# SAC hyper-parameters (identical to nominal)
...
class HoverMetricsCallback(BaseCallback):
    ...
    def __init__(self, print_freq: int = PRINT_FREQ, verbose: int = 0):
        ...
        self._thrust_factors = deque(maxlen=100)
    ...
    def _on_step(self) -> bool:
        ...
        for info in self.locals.get("infos", []):
            ...
            # DR: thrust factor
            if "thrust_factor" in info:
                self._thrust_factors.append(info["thrust_factor"])
        ...
        if self.num_timesteps % 1000 == 0:
            ...
            if self._thrust_factors:
                self.logger.record("custom/mean_thrust_factor",
                                   float(np.mean(self._thrust_factors)))
            if self._ep_rewards:
        ...
        if self.num_timesteps - self._last_print >= self.print_freq:
            ...
            mean_tf = (float(np.mean(self._thrust_factors))
                       if self._thrust_factors else float("nan"))
            print(
                f"  step {self.num_timesteps:>10,} | "
                f"reward={mean_r:+8.2f} | "
                f"ep_len={mean_l:7.1f} | "
                f"hover_err={mean_he:.4f} | "
                f"thrust_f={mean_tf:.2f} | "
                f"fps={fps:6.0f}"
            )
...
def make_train_env(gui: bool = False):
    """Return a function that creates a Monitor-wrapped DR training env."""
    def _init():
...
def make_eval_env():
    """Return a function that creates a Monitor-wrapped DR evaluation env."""
    def _init():
        env = GoalConditionedHoverEnvDR(gui=False)
        return Monitor(env)
    return _init
...
def train(total_steps: int, log_dir: str, gui: bool = False):
    ...
    print(f"\n{'='*70}")
    print(f"  DR Goal-Conditioned Hover — SAC Training")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train SAC for DR goal-conditioned Crazyflie hover",
    ...
    parser.add_argument(
        "--log-dir", type=str, default="logs_hover_sac_dr",
