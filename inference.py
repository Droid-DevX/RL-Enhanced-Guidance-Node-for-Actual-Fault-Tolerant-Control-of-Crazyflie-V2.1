"""
inference.py — Deployment / Testing Script for Goal-Conditioned Hover
=====================================================================
Loads a trained SAC model, takes a target height from the user via the
terminal, and runs the Crazyflie v2 drone in PyBullet to hover at that
height.  Prints hover error every 10 steps.

Usage:
    python3 inference.py                          # uses best model
    python3 inference.py --model path/to/model    # custom model path
    python3 inference.py --gui                    # with PyBullet GUI
    python3 inference.py --episodes 5             # run 5 episodes
...
def run_inference(model_path: str, gui: bool = True,
    ...
    """
    Load the trained model and run inference episodes.
    ...
    for ep in range(num_episodes):
        ...
        for i in range(4):
            ...
            axes[i].grid(True)
            if i == 0:
                axes[i].set_title(f"SAC Control - Motor RPMs (Target Z: {z_target}m)")

        axes[4].plot(steps_arr, z_err_array, color='red', label='Z Error (m)')
        axes[4].set_title("SAC Control - Hover Z-Error over Time")
...
# ──────────────────────────────────────────────────────────────────────────────
def main():
    # Default model path: best model from training
    script_dir = os.path.dirname(os.path.abspath(__file__))
    default_model = os.path.join(script_dir, "logs_hover_sac", "best",
                                 "best_model")

    parser = argparse.ArgumentParser(
        description="Run trained SAC hover model on Crazyflie v2",
    ...
    if not os.path.exists(model_file):
        ...
        print(f"    Train first:  python3 train_nominal.py")
        print(f"    Or specify:   python3 inference.py --model <path>\n")
        sys.exit(1)
